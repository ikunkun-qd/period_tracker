...
class CalendarPage extends GetView<CalendarController> {
        ...
        calendarBuilders: CalendarBuilders(
          ...
          markerBuilder: (context, day, events) {
            return Row(
              ...
              children: controller.getDayMarkers(day),
            );
          },
        ),
      ),
  ...
  Widget _buildDayCell(DateTime day, {bool isSelected = false, bool isToday = false}) {
    ...
    if (isSelected) {
      ...
      if (backgroundColor == const Color(0xFFE91E63)) { // 经期颜色
        textColor = Colors.white;
      } else if (backgroundColor == const Color(0xFF4CAF50)) { // 排卵期颜色
        textColor = Colors.white;
      } else if (backgroundColor == const Color(0xFF81C784)) { // 易孕期颜色
        textColor = Colors.white;
      } else if (backgroundColor == const Color(0xFF2196F3)) { // 安全期颜色
        textColor = Colors.white;
      } else {
  ...
  /// 构建图例项
  Widget _buildLegendItem(String label, Color color, {bool isDashed = false, bool showDot = false}) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Container(
  ...
  Widget _buildQuickActions() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('快速操作', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
